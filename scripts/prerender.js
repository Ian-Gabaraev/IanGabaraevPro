...
function getBodyContent(route) {
  if (route.path === '/blog') {
    ...
    return `<main><h1>Blog</h1><p>${escapeHtml(route.description)}</p>${
      sorted.map(p =>
  ...
  if (post) {
    return `<main><article><h1>${escapeHtml(post.title)}</h1>` +
...
function generateHTML(route) {
  ...
  html = html.replace(/<meta property="og:url" content="[^"]*"/, `<meta property="og:url" content="${BASE_URL}${route.path}"`);
  ...
  html = html.replace(/<meta property="og:type" content="[^"]*"/, `<meta property="og:type" content="${route.og.type}"`);
  ...
  html = html.replace(/<meta property="twitter:url" content="[^"]*"/, `<meta property="twitter:url" content="${BASE_URL}${route.path}"`);
